...
class PDF extends FPDF
{
    ...
    function crearTablaUsuarios($header, $data)
    {
        ...
        foreach ($data as $row) {
            ...
            $this->Cell($w[5], 6, $row['id_rol'] ?? '', 'LR', 0, 'C', $fill);
            $this->Cell($w[6], 6, $row['id_estado'] ?? '', 'LR', 0, 'C', $fill);
            $this->Ln();
            $fill = !$fill;
        }
...
// Obtener datos
$usuarios = consultarUsuariosCRUD($pdo);
