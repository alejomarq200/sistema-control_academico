document.addEventListener("DOMContentLoaded", function () {
  ...
  buscarGradoxNivel();
  document
    .getElementById("form-RegisterActividad")
    .addEventListener("submit", function (event) {
      event.preventDefault(); // Prevenir el envío del formulario

      let isSubmitting = true;
      var gradoActividad = document
        .getElementById("gradoActividad")
        .value.trim();
      var profesorActividad = document
        .getElementById("profesorActividad")
        .value.trim();
      ...
      if (!gradoActividad || gradoActividad == "Seleccionar") {
        document.getElementById("ErrorGradoActividad").textContent =
          "El grado de la actividad es obligatorio";
        isSubmitting = false;
      } else {
        document.getElementById("ErrorGradoActividad").textContent = "";
      }
      ...
      if (!asignatura || asignatura == "Seleccionar") {
        document.getElementById("ErrorAsignatura").textContent =
          "Seleccione una asignatura correcta";
        isSubmitting = false;
      } else {
        document.getElementById("ErrorAsignatura").textContent = "";
      }
      ...
      if (!actividad) {
        document.getElementById("ErrorActividad").textContent =
          "La actividad se encuentra es obligatoria";
        isSubmitting = false;
      } else {
        document.getElementById("ErrorActividad").textContent = "";
      }
      ...
      if (!profesorActividad || profesorActividad == "Seleccionar") {
        document.getElementById("ErrorProfesorActividad").textContent =
          "La información del profesor es obligatoria";
        isSubmitting = false;
      } else {
        document.getElementById("ErrorProfesorActividad").textContent = "";
      }
      ...
      if (!tipoContenido || !regexName.test(tipoContenido)) {
        document.getElementById("ErrortipoContenido").textContent =
          "El tipo de cntenidoes obligatorio";
        isSubmitting = false;
      } else {
        document.getElementById("ErrortipoContenido").textContent = "";
      }
